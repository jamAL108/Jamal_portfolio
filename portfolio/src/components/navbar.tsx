...
const Navbar = () => {
    ...
    return (
        ...
        <>
            ...
            <div className='base:flex z-[10000000] fixed backdrop bottom-0 bl:hidden w-full h-[64px] items-center justify-around'>
                {/* <div className='absolute top-0 w-full flex items-center justify-center flex-col gap-6 mt-6'>
                    <Image src='/images/JamalPhoto.jpg' alt='asdf' width={32} height={32} className='rounded-full' />
                    {
                        theme === 'dark' ? (
                ...
                <Link href={'/projects'}>
                    <Presentation className={`${pathname !== null && pathname.includes('/projects') === true ? 'text-main' : 'hover:text-foreground text-muted-foreground'} cursor-pointer transition-all duration-200 ease-in-out`} size={24} />
                </Link>
                <Link href={'/resume'}>
                    <IoDocumentTextOutline className={`${pathname !== null && pathname.includes('/resume') === true ? 'text-main' : 'hover:text-foreground text-muted-foreground'} cursor-pointer transition-all duration-200 ease-in-out`} size={24} />
                </Link>
            </div>
